...
def visualize_boxes_and_labels_on_image_array(image,
  ...
  for box, color in box_to_color_map.items():
    ...
    if original is not None:
        ...
        crop_xmin = image_width*xmin
        crop_ymin = image_height*ymin
        crop_xmax = image_width*xmax
        crop_ymax = image_height*ymax
        image_name = str(index)+'.jpg'
        if index < 10:
            image_name = '00'+str(index)+'.jpg'
        elif index < 100:
            image_name = '0'+str(index)+'.jpg'
        cropped = original.crop((crop_xmin,crop_ymin, crop_xmax, crop_ymax))
        cropped.save(os.path.join('output/result', image_name))
        index += 1
   ## add code
